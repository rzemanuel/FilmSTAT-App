    ...
    with createProjectSection:
        ...
        if project_name and source and target and spectrum:
            if st.button("Sumbit", key='submit' ):
                url = 'https://34.125.72.164:8000/projects/'
                headers = {
...
def GetLUT_Clicked(project_id):
    ...
    url = f'https://34.125.72.164:8000/LUT/?project_id={project_id}'
    headers = {
        'accept': 'application/json',
        'Authorization': f'Bearer {st.session_state["token"]}',
        }
...
def train_model(args):
    ...
    url = f'https://34.125.72.164:8000/ml/?lut_size={lutsize}&project_id={project_id}'
    headers = {
        'accept': 'application/json',
        'Authorization': f'Bearer {st.session_state["token"]}',
        'Content-Type': 'application/json'}
...
def send_files(files_name_bytes, source_target,):
    ...
    url = f'https://34.125.72.164:8000/images/?source={source_target}&project_id={st.session_state["project_id"]}'
...
def show_load_files_page():
    ...
    with loadFilesSection:
        ...
        if st.button('reset image folders', key='reset_image_folder'):
            # Build the multipart/form-data request.
            url = f'https://34.125.72.164:8000/images/?project_id={st.session_state["project_id"]}'
...
def verify_credentials(username, password):
    url = 'https://34.125.72.164:8000/token'
    headers = {
        'accept': 'application/json',
...
def create_tab_data(s_t,chart_x,chart_y):
    if s_t == "Source":
        ...
        source = 'false'
    url = f'https://34.125.72.164:8000/data/?source={source}&chart_x={chart_x}&chart_y={chart_y}&project_id={st.session_state["project_id"]}'
    headers = {"Content-Type": "application/json",
               'Authorization': f'Bearer {st.session_state["token"]}',
                'accept': "application/json"}
    data =  {
...
def get_images_cached(source, project_id, token,):
    if source:
        ...
        s_t = 'false'
    url = f'https://34.125.72.164/images/oneimage?source={s_t}&project_id={project_id}'
    headers = {
        'accept': 'application/json',
...
def image_comparison():
    ...
    if left_image is not None and right_image is not None:
        ...
        with st.container():
            ...
            url = f'https://34.125.72.164:8000/LUT/?project_id={st.session_state["project_id"]}'
            headers = {
                'accept': 'application/json',
                'Authorization': f'Bearer {st.session_state["token"]}',
            }
